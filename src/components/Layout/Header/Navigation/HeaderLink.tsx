...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link
        href={item.href}
        className={`text-l flex font-medium duration-300  ${path === item.href ? "text-primary " : " text-black/70 dark:text-white/50 hover:text-primary dark:hover:text-primary"
          }`}
      >
        {item.label}
        {item.submenu && (
