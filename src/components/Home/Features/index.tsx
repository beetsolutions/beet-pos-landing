    ...
    return (
        <section className="bg-primary/15">
            <div className='container  mx-auto lg:max-w-screen-xl md:max-w-screen-md' id="about-section">
                <div className='text-center mb-14 ml-5 mr-5' >
                    {/*<p className='text-primary text-lg font-normal mb-3 tracking-widest uppercase'>Features</p>*/}
                    <h2 className='text-3xl lg:text-5xl font-semibold text-black dark:text-white lg:max-w-60% mx-auto'>Simple, Secure & State of the art Solutions
                        For Commerce</h2>
                </div>
                <div className='grid sm:grid-cols-2 lg:grid-cols-4 gap-y-20 gap-x-5 mt-32 ml-5 mr-5'>
                    {FeaturesData.map((items, i) => (
                        <div className='p-8 relative rounded-3xl bg-gradient-to-b from-black/5 to-white dark:from-white/5 dark:to-black' key={i}>
                            <div className='work-img-bg rounded-full flex justify-center absolute -top-[20%] sm:top-[-40%] md:top-[-25%] lg:top-[-20%] sm:left-[30%] left-[35%]'>
                                <Image src={items.imgSrc} alt={items.imgSrc} width={110} height={10} />
                            </div>
